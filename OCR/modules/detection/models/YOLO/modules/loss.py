...
from .utils.general import is_parallel, bbox_iou
...
class YoloLoss():
    def __init__(self,
        ...
        self.obj_pw = obj_pw
        self.cls_pw = cls_pw
        self.anchor_t = anchor_t # anchor-multiple threshold
        self.gr = gr # giou loss ratio (obj_loss = 1.0 or giou)
        self.nc = num_classes

        device = next(model.parameters()).device  # get model device

        # Define criteria
        self.BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([self.cls_pw])).to(device)
        self.BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([self.obj_pw])).to(device)
